...
# 1. ¿Por qué Tidymodels?

## 1.1. Un poquito de historia {auto-animate="true"}

<div style="margin-top:50px; padding:0px; margin-bottom=0px">
![](Imagenes/Max_Kuhn.jpeg){width="400" height="400" fig-align="center"}
</div>
...
## 1.1. Un poquito de historia (2) {auto-animate="true"}
...
-   Tiene una sintáxis fácil y fluida que se enfoca más en resolver necesidades que en la programación.
:::
:::
:::

## 1.1. Un poquito de historia (3) {auto-animate="true"}
...
## 1.2. Un breve ejemplo {auto-animate="true"}
...
## 1.2. Un breve ejemplo (2) {auto-animate="true"}
...
# 2. ¿De qué se compone Tidymodels?

## 2. ¿De qué se compone Tidymodels? {auto-animate="true"}
...
::: incremental
::: {style="font-size:32px; margin-left:40px;"}
...
-   Además de nuestra variable *Var_respuesta*, tenemos otras 25 variables: de cognición, bioquímicas, genéticas, etc.
:::
:::
...
# 4. Preparando la partición de datos con Rsample

## 4.1. Cuidado con los datos {auto-animate="true"}
...
## 4.1.1. Remuestreo con *RSample* {auto-animate="true"}

-   Crearemos una partición de la muestra en unos datos para “entrenar” el modelo y la otra para “evaluarlo”.

::: {layout-ncol="2"}
![](Imagenes/esquema_training_test.jpeg){fig-align="center" width="500"}

![](Imagenes/logo_rsample.png){fig-align="center" width="300"}
:::

## 4.1.2. Algunas funciones importantes en *RSample* {auto-animate="true"}
...
## 4.2. Estratificación {auto-animate="true"}
...
## 4.3. Deshacer la partición {auto-animate="true"}
...
## 4.4. Folds, creando conjuntos de validación {auto-animate="true"}
...
# 5. Pre-procesamiento de datos con Recipes

## 5.1. Pre-procesamiento de datos {auto-animate="true"}

::: incremental
1.  Imputación
2.  Normalización
3.  Re-equilibración de grupos
4.  Conversión
:::
...
## 5.2. ¿Cómo cocinamos una receta? {auto-animate="true"}
...
## 5.2.1. Hagamos un ejemplo {auto-animate="true"}
...
## 5.2.2. Imputación múltiple {auto-animate="true"}
...
## 5.2.4. Remuestrear {auto-animate="true"}
...
## 5.3. Caso práctico: Receta {auto-animate="true"}
...
# 6. ¿Dónde está el modelo? Creando modelos con Parsnip

## 6.1. Modelos a la carrera con Parsnip {auto-animate="true"}
...
## 6.2. Empezamos a modelar {auto-animate="true"}
...
* Cada uno de los motores cuenta con características computacionales distintas:
  + Por ejemplo, si hacemos una regresión lineal con *glm*, el modelo correrá dentro de nuestro ordenador.
  + En cambio, si usamos los motores *spark* o *h2o*, el modelo se compilará en sus servidores externos.
...
## 6.3. Caso práctico: modelo de Random Forest {auto-animate="true"}
...
-   La preparación de la receta y el ajuste del modelo pueden ejecutarse utilizando una única llamada a `fit()`.
-   Si tiene configuración personalizada de parámetros de ajuste, éstos pueden definirse utilizando una interfaz más sencilla cuando se combinan con *tune*.
...
# 8. Eligiendo métricas con Yardstick {auto-animate="true"}
...
<div style="font-size:26px; margin-left:0px;">
[Más información](https://yardstick.tidymodels.org/articles/custom-metrics.html)
</div>
:::
:::
...
# 9. Eligiendo hiperparámetros: Tune y Dials

## 9.1. Eligiendo hiperparámetros: Tune y Dials {auto-animate="true"}
...
::: incremental
1.  Creamos unas réplicas del conjunto de entrenamiento, Rsample con el objeto *cv_Folds*

2.  Elegimos un modelo con Parsnip y dejamos sus hiperparámetros en abierto con `tune()`.

3.  Creamos una cuadrícula (grid) con diferentes posibles combinaciones de los hiperparámetros

4.  Ponemos todo junto con la función `tune_grid()`.
:::

## 9.2. Montando un espacio de hiperparámetros: familia `grid_` {auto-animate="true"}
...
-   `grid_max_entropy`: propondrá una combinación de hiperpárametros que garantice que se cubrirá todo el espectro, con fin de evitar mínimos locales en la conversión de algoritmos.
-   `grid_latin_hypercube`: propondrá una matriz.
:::

## 9.2.1. Grid_regular() {auto-animate="true"}
...
## 9.2.2. Grid_max_entropy() {auto-animate="true"}
...
## 9.3. Hiperparámetros: Dial & Tune + Yardstick {auto-animate="true"}
...
# 10. La conversión de todo: fit() y predict()

## 10. La conversión de todo: fit() y predict() {auto-animate="true"}
...
2.  Un tibble/dataframe con la nueva predicción (debe contener las mismas variables)

3.  Un tipo de predicción en los casos de clasificación, si queremos las probabilidades de cada categoría o la categoría predicha directamente
:::
...
## 10.1. La función last_fit() {auto-animate="true"}

::: incremental
-   La función `last_fit()` permite tener todo unificado en un solo tibble. Recibe directamente los datos de entrenamiento y los datos de evaluación y crea sus métricas y sus predicciones.
:::
...
## 11. Threshold performance {auto-animate="true"}

::: incremental

- El concepto de **rendimiento del umbral** *(threshold performance)* es crucial a la hora de evaluar y ajustar modelos de clasificación binaria (Sí/No).

- El **umbral** es el valor de probabilidad por encima del cual un resultado predicho se considera de clase positiva. 
...
::: incremental
- Por defecto, muchos modelos utilizan un **umbral de 0,5.** 

- Sin embargo, en casi todos los casos, **el umbral óptimo estará en otro punto.** 

- Este “óptimo" lo tendremos que definir  en función de los objetivos y requisitos específicos del problema.
:::

## 11. Threshold performance {auto-animate="true"}

::: incremental
- Ajustar el umbral permite elegir entre precisión y la sensibilidad del modelo.

- Bajar el umbral aumenta la sensibilidad *(recall)* pero disminuye la precisión, y viceversa. 

- **Básicamente, el umbral óptimo depende de la importancia relativa de los falsos positivos y los falsos negativos en una aplicación concreta.**
:::

## 11. Threshold performance {auto-animate="true"}

::: incremental
-   En tidymodels está la librería `probably` con la función `threshold_perf()`, la cual nos permite, dadas las predicciones del modelo, ver cómo se desenvuelve este umbral.
:::
...
# 12. workflow_set() para unificarlas todas

## 12.1. workflow_set() {auto-animate="true"}

::: incremental
-   Supongamos que queremos evaluar 2 recetas con 1 modelo de Random Forest:

::: {style="margin-left:50px; font-size:35px;"}
1.  Una con solo las variables 'Escala'
2.  Otra con solo las variables 'Cognición'
:::

-   Queremos dejarlo abierto a que puedan haber más.
:::
...
## 12.2. workflow_set() {auto-animate="true"}
...
## 12.3. Métricas de todos los modelos {auto-animate="true"}
...
## 12.4. Curvas roc de todos los modelos {auto-animate="true"}
...
- En el contexto del aprendizaje automático, las características o variables se consideran jugadores, y el juego consiste en **cuánto contribuye cada característica a la predicción de un modelo.**
...
::: incremental
- Los *valores de Shapley* son sencillamente las **marginales de cada variable dentro del modelo.** 

- Compilarlos es computacionalmente extenso, ya que hay que hacer cada combinación de valores para una fila y evaluar de cuánto es su marginal. 

- El valor final llamado *Shap value* es **el promedio de todas las aportaciones de una variable marginal.**
...
::: incremental
- Aparte de su extenso coste computacional, todo Shap value asume que está utilizando un modelo de caja negra, de manera que si el modelo no está bien especificado es posible que esté dando indicaciones erróneas.
